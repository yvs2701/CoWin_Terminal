...
def findByName(dist: int, min_age: int, max_age: int, urlParams: str = "") -> bool:
    '''Check the availibility of the vaccines in the specified district'''
    # fetch data
    today = date.today()
    URL = 'https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByDistrict?district_id={}&date={}'.format(dist, today.strftime("%d-%m-%Y"))
    URL = URL + urlParams
